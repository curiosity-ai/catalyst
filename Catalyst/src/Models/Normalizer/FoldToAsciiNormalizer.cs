...
{
    public class FoldToAsciiNormalizer : INormalizer, IProcess
    {
        public string Type => typeof(FoldToAsciiNormalizer).FullName;
        public Language Language => Language.Any;
        public string Tag => "";
        public int Version => 0;

        public static async Task<FoldToAsciiNormalizer> FromStoreAsync(Language language, int version, string tag)
        {
            return await Task.FromResult(new FoldToAsciiNormalizer());
        }
        ...
        private static string ReplaceUnicodeChar(char c)
        {
            if (Map.TryGetValue((int)c, out string replacement))
            {
                return replacement;
            }
            return c.ToString();
